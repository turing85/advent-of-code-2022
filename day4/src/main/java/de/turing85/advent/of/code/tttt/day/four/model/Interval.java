...
public record Interval(int lower, int upper) {
   ...
   * <p>
   * If two intervals are equal, they also contain each other.
   *
  ...
  /**
   * Returns {@code true}, if {@code this} {@link Interval} overlaps with {@code that}
   * {@link Interval}.
   *
   * @param that the other {@link Interval} to check
   *
   * @return {@code true} if and only if
   *     {@code (this.lower() <= that.lower() && that.lower() <= this.upper()) || (that.lower() <=
   *     this.lower() && this.lower() <= that.upper())}
   */
   ...
   *
   * <p>The order for this operation does not matter, it returns {@code true} if and only of
   * {@code lhs.fullyContains(rhs) || rhs.fullyContains(lhs)}.
   *
   ...
   *
   * @return {@code true} if and only if
   *     {@code (lhs.lower() <= rhs.lower() && rhs.lower() <= lhs.upper()) || (rhs.lower() <=
   *     lhs.lower() && lhs.lower() <= rhs.upper())}
   */
