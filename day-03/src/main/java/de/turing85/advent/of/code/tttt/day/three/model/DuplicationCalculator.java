 ...
 *
 * <p>The duplication factor is calculated by:
 *
 * <ul>
 *   <li>determine which item is duplicated in both containers of each rucksack (must be exactly
 *       one)
 *   <li>and then all priorities of all duplicated items over all rucksacks are summed up.
 * </ul>
 */
public class DuplicationCalculator {
  ...
  public int calculateDuplication(List<Rucksack> rucksacks) {
    if (rucksacks.stream()
        .map(Rucksack::getItemPrioritiesInBothCompartments)
